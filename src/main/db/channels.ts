...
export async function ipcInvoke(name: string, args: ReadonlyArray<unknown>): Promise<void> {
...
export async function doShutdown(): Promise<void> {
  log.info(`data.shutdown: shutdown requested. ${activeJobCount} jobs outstanding`);
  ...
  ({ promise: shutdownPromise, resolve: resolveShutdown } = explodePromise<void>());
